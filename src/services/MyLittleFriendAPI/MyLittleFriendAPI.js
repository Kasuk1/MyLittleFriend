...
export const MyLittleFriendAPI = {
  ...
  async signUp(data) {
    let url;
    if (data.avatar_url) {
      const uploadImageResponse = await MyLittleFriendAPI.uploadFile(
        data.avatar_url
      );
  ...
  async signIn(data) {
    ...
    headerPut.Authorization = `Bearer ${user.data.token}`;
    headerDelete.Authorization = `Bearer ${user.data.token}`;
    return user;
  },
